...
# tags: search_id, foreign = searches
# gif_id, foreign = gifs
user_collection = db.Table('user_collection', db.Column('user_id', db.Integer, db.ForeignKey('gifs.id')),db.Column('collection_id',db.Integer, db.ForeignKey('collections.id')))
# user_id, foreign = gifs
# collection_id, foreign = collections
...
def get_gifs_from_giphy(search_string):
    ...
    giphy_response = requests.get(baseurl, params=params)
    #print giphy_response
    giphy_data = json.loads(giphy_response.text)
    giphy_data_extracted = giphy_data["data"]

    return giphy_data_extracted
...
def get_or_create_gif(title, url):
    ...
    if gif:
        return gif
        #checks if instance of gif exists in db
        ...
        db.session.commit()
        return gif
        #adds instance to db if does not exist
...
def get_or_create_search_term(term):
    ...
    current_term = db.session.query(SearchTerm).filter_by(term=term).first()
    if current_term:
        return current_term
    ...
    # And the SearchTerm instance that was got or created should be returned.
        ...
        current_term = SearchTerm(term = term)
        gif_call = get_gifs_from_giphy(term)
        #print(search)
        #print(gif_call)

        for item in gif_call:
            gif = get_or_create_gif(title = item["title"], url = item["embed_url"])
            #print(gif)
            current_term.gifs.append(gif)
...
def index():
    ...
    if form.validate_on_submit():
        ...
        current_term = get_or_create_search_term(form.search.data)
        return redirect(url_for("search_results", search_term = current_term.term))
...
def create_collection():
    ...
    if request.method == 'POST':
        active_gifs = form.gif_picks.data
        gifs_for_collection = [get_gif_by_id(int(id)) for id in active_gifs]
        get_or_create_collection(name = form.name.data, current_user = current_user, gif_list = gifs_for_collection)
